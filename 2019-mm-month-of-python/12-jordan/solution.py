...
def get_held_rain(x):
    ...
    # array of len(max(x)) for the gap and ingap counters
    totaltotal = 0
    for level in range(max(x)):
        ...
        for i, b in enumerate(building[:-1]):
            ...
            #     continue
            if (not ingap) and b and (not building[i + 1]):
                ingap = True
            ...
            #     gap = 0
            #     ingap = False
            elif ingap and not b and (building[i + 1]):
                gap += 1
                total += gap
                gap = 0
                ingap = False
            elif ingap and not b and (not building[i + 1]):
                gap += 1
        # print(total)
        totaltotal += total
    return totaltotal
